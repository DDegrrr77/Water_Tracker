...
1. **첫 실행 (단일 사용자, v0.2.0+)**
   - 사용자 선택 화면 없이 곧바로 메인 대시보드(오늘의 수분 기록 탭)로 진입
   - 데이터는 단일 사용자 키(`hydration_settings`, `hydration_logs`)에 저장
   - v0.1.x 이전 데이터가 남아있으면 앱 마운트 시 1회 자동 마이그레이션
2. **수분 기록 (홈 탭)**
...
4. **설정 및 데이터 관리 (설정 탭)**
   - 몸무게·활동량·기상/취침 시간·음료별 기본량/반영률·알림 간격 변경
...
- **단일 사용자 (v0.2.0+)**: 다중 사용자(UserSelection) 제거 — 브라우저 `localStorage` 기반
  단일 사용자 프로필 (백엔드/DB 없음, 사용자별 분기 스토리지 없음)
- **PWA 기반 구조**: `manifest.json` + `sw.js`(패스스루) + SVG 아이콘으로 설치 가능 형태
- **반응형 앱 프레임(v0.2.0)**: 최상위가 중앙 정렬 모바일 카드 뷰(`max-w-md mx-auto`) +
...
└── src/
    ...
    ├── App.tsx               # 루트: 단일 사용자 HydrationProvider + 탭 네비게이션 + PADO 브릿지 마운트 + 스크롤 반응 헤더/네비
    ├── types.ts              # 도메인 타입 (DrinkType/User(레거시)/UserSettings/HydrationLog/Tab)
    ...
    │   ├── SettingsTab.tsx   # 설정: 프로필·음료 설정·알림·백업/복구
    │   └── Toast.tsx         # 수분 부족 경고 토스트 (3초 자동 숨김)
    ├── hooks/
    │   ├── useLocalStorage.ts # localStorage ↔ React state 동기화 훅 (변경 이벤트 발행/구독)
    │   └── usePadoBridge.ts   # PADO 부모 창 양방향 동기화 브릿지 훅 (단일 사용자)
    ├── lib/
    │   ├── utils.ts          # cn(), 권장량 계산 함수
    │   ├── notifications.ts  # 브라우저 알림 권한/발송
    │   ├── storage.ts        # localStorage 공용 유틸 + aquaflow:storage-change 이벤트 + 레거시 마이그레이션
    │   └── pado.ts           # PADO 프로토콜/페이로드 정의 (단일 사용자 스키마, summary 계산, 복원)
    └── store/
        └── HydrationContext.tsx # 전역 상태 Provider(단일 사용자) + useHydration() 훅
```
...
main.tsx
  └─ App.tsx
       └─ HydrationProvider (단일 사용자, props 없음)
            └─ AppContent (usePadoBridge 마운트)
                 ├─ HomeTab        ┐
...
### 4.2 `User` — ⚠️ 레거시 타입 (v0.1.x 전용, 더 이상 생성/관리 안 함)

```ts
/** @deprecated v0.2.0부터 단일 사용자 모델로 전환 — 마이그레이션 파싱에만 사용 */
...
### 4.3 `DrinkSetting` · `UserSettings` — 단일 사용자 설정
...
### 5.1 localStorage 저장 키 (v0.2.0 단일 사용자)

| 키 | 값 타입 | 생성 시점 | 삭제 시점 |
|---|---|---|---|
| `hydration_settings` | `UserSettings` | 최초 실행 시 (기본값) | 없음 (단일 사용자 전용) |
| `hydration_logs` | `HydrationLog[]` | 최초 기록 시 (빈 배열 기본) | 없음 (단일 사용자 전용) |

> ⚠️ **레거시 키** (`app_users`, `hydration_settings_{userId}`, `hydration_logs_{userId}`):
> v0.1.x에서 사용되던 키. 앱 마운트 시 `migrateLegacyData()`가 기본/활성 사용자의 데이터를
> 위 신규 키로 **1회 자동 복사**합니다. 레거시 키는 삭제하지 않음 (비파괴 — 데이터 유실 방지).

### 5.2 영속화 메커니즘 (`useLocalStorage` 훅 + `lib/storage.ts`)

- 초기화: `useState` 지연 초기화로 `localStorage`에서 JSON 파싱 → 없으면 `initialValue`
- 마이그레이션: `HydrationProvider` 마운트 시 `migrateLegacyData(defaultSettings)` 호출
  (멱등 — 신규 키 `hydration_logs`가 존재하면 무동작)
...
**① 레거시 데이터 마이그레이션** (`HydrationContext` 마운트 → `migrateLegacyData`)
1. 신규 키 `hydration_logs`가 이미 존재하면 종료 (멱등)
2. `app_users`의 **마지막(기본/활성) 사용자** id 결정 — 없으면 `hydration_logs_*`/`hydration_settings_*`
   키 스캔으로 대상 탐색
3. 대상 사용자의 `hydration_settings_{id}` → `hydration_settings`, `hydration_logs_{id}` →
   `hydration_logs`로 복사
4. 레거시 키는 유지 (비파괴)
...
- 백업: `{ settings, logs, version: '2.0' }`을 JSON 파일로 다운로드
  (`water_{yyyyMMdd_HHmmss}.json`)
- 복구: 파일 업로드 → 파싱 → `settings`와 `logs` 키가 모두 있으면 기본값과 병합 후 교체 저장
  (신규 필드 `reminderInterval` 등 누락 대비, 스키마 심층 검증은 없음 — 기술 부채 참고)
...
- Provider는 `App.tsx`에서 props 없이 마운트됨 (`<HydrationProvider>`) — 단일 사용자 전용
- PADO 브릿지(`usePadoBridge`)는 `AppContent` 내부(Provider 안)에서 마운트되어
  `settings`/`setSettings`/`logs`/`setLogs`를 전달받음
...
- **반응형 앱 프레임 (v0.2.0, 필수)**: 넓은 화면·PADO iframe에서 모바일 규격을 유지한다.
  ...
  - 이 구조는 App.tsx에서 **단일 프레임(사용자 선택 화면 없음)**으로 유지 (새 화면 추가 시 동일 패턴 유지)
...
1. **상태 저장 우회 금지**: `settings`/`logs`는 반드시 Context의 `updateSettings`/
   `setSettings`/`addLog`/`setLogs` 경로로 변경. `localStorage` 직접 쓰기 금지
   (예외: `migrateLegacyData`의 레거시 키 복사, `writeLocalJson` 공용 유틸)
2. **데이터 스키마 하위 호환**: `localStorage`에 저장된 기존 데이터를 깨는 타입 변경은
   `migrateLegacyData`와 함께 수행 (v0.2.0: 멀티 유저 → 단일 사용자 키 마이그레이션 완료)
3. **Provider 재구조화 주의**: `HydrationProvider`는 단일 사용자 전용 — `userId` props를
   다시 추가하거나 사용자별 키(`hydration_*_{userId}`)를 부활시키지 말 것
...
단일 사용자 **설정/기록 데이터를 양방향으로 동기화**하며, PADO 에이전트(Hermes)는
...
- **단일 사용자 (v0.2.0)**: `users`/사용자별 `{userId}` 분기 없이 단일 `settings`/`logs`만 동기화

관련 파일:
- `src/lib/pado.ts` — 프로토콜 상수, 페이로드 타입, `buildPadoPayload`/`buildSummary`/`restoreFromPadoPayload`
- `src/lib/storage.ts` — localStorage 공용 유틸 + `aquaflow:storage-change` 이벤트 + 레거시 마이그레이션
- `src/hooks/usePadoBridge.ts` — 브릿지 훅 (AppContent 내부, Provider 안에서 마운트)
...
### 7.3 통합 동기화 페이로드 (`PadoPayload`) — 단일 사용자 스키마
...
interface PadoPayload {
  ...
  settings: {
    target: number;             // 일일 목표량 (ml) = calculateTotalRecommended(weight, activityLevel)
    cupSize: number;            // 컵 1회 기본 용량 (ml) = drinkSettings.water.amount
    reminderInterval: number;   // 알림 간격 (분)
  };
  logs: Array<{
    id: string;
    timestamp: string;          // ISO 8601
    amount: number;             // 수분 반영량 (ml) = hydrationAmount
    type: string;               // DrinkType (현재 스키마: 'water')
  }>;
  summary: {
    todayIntake: number;        // 오늘 총 섭취량 (ml)
    target: number;             // 일일 목표량 (ml)
    achievementRate: number;    // 달성률 (%) — 최대 100% 캡
...
```json
{
  ...
  "logs": [{ "id": "a1b2c3d", "timestamp": "2026-08-28T00:30:00.000Z", "amount": 250, "type": "water" }],
  "summary": { "todayIntake": 1250, "target": 2000, "achievementRate": 63, "todayLogCount": 5 }
}
```

`summary`는 단일 사용자 기준으로 계산됩니다.
`todayIntake` = 오늘 로그의 `hydrationAmount` 합계, `target` =
`calculateTotalRecommended(weight, activityLevel)`, `achievementRate` = 최대 100%로 캡.

`buildPadoPayload(settings, logs)` — 내부 상태 → 외부 페이로드 변환.
`restoreFromPadoPayload(payload, fallbackSettings)` — 외부 페이로드 → 내부 상태 복원
(target → 체중 역산, cupSize → `drinkSettings.water.amount`, timestamp ISO → epoch ms).
...
2. **INIT_RESPONSE 수신 시**:
   - `payload`에 데이터 있음(`settings` 또는 `logs` 존재) → `restoreFromPadoPayload`로
     단일 settings/logs를 복원하여 `setSettings`/`setLogs`로 **즉시 반영** (localStorage + 화면).
...
3. **데이터 변경 감지**: `useLocalStorage`/`writeLocalJson`/`removeLocalKey`가 발행하는
   ...
   `PADO_DATA_SYNC` 발신
   - 수분 기록 추가/삭제, 설정(목표·컵 용량·알림 간격 등) 수정 시 즉시 동기화
...
- **로그 규격**: 임베딩 시 `[PADO] PADO_DATA_INIT_REQUEST sent ...`,
  `[PADO] PADO_DATA_SYNC sent (logs=... todayIntake=...ml ...)` 등으로 콘솔에 기록 (검증용)
